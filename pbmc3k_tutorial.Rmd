...
This tutorial will be based off of the `Seurat` tutorial provided by the [Satija lab](https://satijalab.org/seurat/vignettes.html). `Seurat` is designed much more broadly as a toolkit for single-cell genomics, so we will focus on aspects specific to dimension reduction, clustering, and differential gene expression. The data we use is output from cellranger, available for download [here](https://s3-us-west-2.amazonaws.com/10x.files/samples/cell/pbmc3k/pbmc3k_filtered_gene_bc_matrices.tar.gz).] It is made up of 2700 samples of Peripheral Blood Mononuclear Cells (PBMCs) with counts of about 14,000 RNA molecules. The rows are genes and the columns are cells. 

Conveniently, `Seurat` does a lot of pre-processing for us and packages together the count matrices that we're working with along with results like dimension reduction coordinates. The dimension reduction is called from other packages, such as the `uwot` package for UMAP. When working with dimension reduction directly (i.e. without the `Seurat` package), you can call your preferred dimension reduction method via, for example, the `umap:uwot` or `prcomp` commands.
